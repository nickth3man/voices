"""
Utility functions for Demucs model.

This module provides utility functions for loading and using the Demucs model,
...
def load_demucs_model(
    ...
    **kwargs
    ...
    """
    Load a Demucs model from a path.
    ...
    logger.info(f"Loading Demucs model from {model_path} on {device}")
    
    try:
        # Load model using the from_pretrained method
        model = DemucsModel.from_pretrained(model_path, device=device, **kwargs)
        logger.info(f"Successfully loaded Demucs model")
        return model
...
def create_demucs_registry_loader() -> Callable:
    """
    Create a loader function for the Demucs model that can be registered with the model registry.
    
    Returns:
        Function that loads a Demucs model from a path
    """
    def load_demucs_for_registry(path, **kwargs):
        """
        Load a Demucs model for use with the model registry.
        ...
        """
        # Load the model
        model = load_demucs_model(path, **kwargs)
        
        # Create a processing function
        def process_audio(mixture, **proc_kwargs):
            """
            Process audio using the loaded Demucs model.
    ...
    return load_demucs_for_registry


def download_pretrained_model(
    output_dir: str,
    model_url: Optional[str] = None,
    model_name: str = "demucs_base",
    force_download: bool = False
) -> str:
    """
    Download a pretrained Demucs model.
    ...
    if model_url is None:
        ...
        model_url = f"https://example.com/models/demucs/{model_name}.pth"
